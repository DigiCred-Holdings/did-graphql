...
test('checkAuthOnly verifies a did:key chain locally, no agent or DB involved', async () => {
  ...
  assert.equal(result.controller, invoker.did)
  assert.deepEqual(result.problems, [])
})

test('checkAuthOnly rejects a tampered did:key leaf, with a PROOF_INVALID problem', () => {
  const tampered = { id: 'urn:zcap:delegated:tampered', controller: invoker.did, invocationTarget: GRAPHQL_ENDPOINT, allowedAction: ['query Evil { x }'] }
...
test('checkAuthOnly rejects a rootCapability whose controller did not sign the leaf (PROOF_INVALID)', async () => {
  ...
  const result = checkAuthOnly(realConfig(invoker.did), { chain: [capability] })
  assert.equal(result.valid, false)
...
test('checkInvocation rejects an invocation signed by someone other than the leaf controller', async () => {
  const capability = await delegateGraphqlZcap(agent, issuer, invoker)
  // Signed by the issuer (root controller), not the invoker (leaf controller) — invalid invoker.
